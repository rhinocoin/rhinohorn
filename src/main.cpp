...
uint256 hashGenesisBlock("0xd2c48c2e5ed7dd037899d02ccb2239d64b5607aecd08c2f91884cd96b8cd057a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Rhinohorn: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Rhinohorn Signed Message:\n";
...
{
    // Rhinohorn: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 105000); // Rhinohorn: 840k blocks in ~4 years
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Rhinohorn: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Rhinohorn: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Rhinohorn: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 2116) { DiffMode = 2; } // Rhinohorn, 100 blocks after first legacy diff adjustment
        }
        else {         
        	if (pindexLast->nHeight+1 >= 14750) { DiffMode = 2; }  //Rhinohorn, 5 days after 27/01/2014 12:00 UTC
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Rhinohorn: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Rhinohorn: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 'r';
        pchMessageStart[3] = 't';
        hashGenesisBlock = uint256("0x105c3fcad08438903b02918538cde8707280e6b0afb154386474e79624cbc3d0");
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "03/09/2014 Dorian Nakamoto accused of being Satoshi Nakamoto";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce = 6163450;
        block.nTime = 1394263890;
        
        if (fTestNet)
        {

            block.nNonce = 13193230 ;
            block.nTime = 1394230681;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x6529aace2fa90fe97fcd4ac617e91b48e43febe22ed3e81fe985e972ce731e43"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // Rhinohorn: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RhinohornMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RhinohornMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RhinohornMiner(CWallet *pwallet)
{
    printf("RhinohornMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Rhinohorn-miner");
    ...
    try { loop {
        ...
        printf("Running RhinohornMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RhinohornMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RhinohornMiner, pwallet));
}
