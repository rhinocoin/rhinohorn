...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Rhinohorn " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    ...
    {"seed1", "199.127.226.163",},
    {"seed2",  "220.239.233.14"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Rhinohorn is probably already running."), addrBind.ToString().c_str());
        else
